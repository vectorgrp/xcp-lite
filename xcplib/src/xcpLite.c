...
static void XcpTriggerDaqList(const tXcpDaqLists *daq_lists, uint16_t daq, const uint8_t *base, uint64_t clock)
{
  ...
  for (hs = ODT_HEADER_SIZE + ODT_TIMESTAMP_SIZE, odt = DaqListFirstOdt(daq); odt <= DaqListLastOdt(daq); hs = ODT_HEADER_SIZE, odt++)
  {
    ...
    *((uint16_t *)&d0[2]) = daq;
    ...
    // Timestamp 32 bit
    if (hs == ODT_HEADER_SIZE + ODT_TIMESTAMP_SIZE)
    { // First ODT always has a 32 bit timestamp
...
#endif
    if (DaqListState(daq) & DAQ_STATE_RUNNING)
    {                                                 // DAQ list active
      XcpTriggerDaqList(daq_lists, daq, base, clock); // Trigger DAQ list
    }
    daq = DaqListNext(daq);
...
#define CRO_DWORD(x) (CRO->dw[x])
  ...
  else
  {
    ...
    if (DBG_LEVEL >= 4 && !async)
      XcpPrintCmd(CRO);
#endif
    if (!isConnected() && CRO_CMD != CC_TRANSPORT_LAYER_CMD)
    { // Must be connected, exception are the transport layer commands
...
/* Not implemented, no gXcp.ProtectionStatus checks */
    ...
    case CC_SHORT_DOWNLOAD:
    {
      ...
      if (size > CRO_SHORT_DOWNLOAD_MAX_SIZE || size > CRO_LEN - CRO_SHORT_DOWNLOAD_LEN)
        error(CRC_CMD_SYNTAX);
      if (!async)
      { // When SHORT_DOWNLOAD is executed async, MtaXxx was already set
    ...
    case CC_SHORT_UPLOAD:
    {
      ...
      if (size > CRM_SHORT_UPLOAD_MAX_SIZE)
        error(CRC_OUT_OF_RANGE);
      if (!async)
      { // When SHORT_UPLOAD is executed async, MtaXxx was already set
...
#error "Supports only 32 bit timestamps"
    ...
    case CC_START_STOP_SYNCH: // prepare, start, stop selected daq lists or stop all
    {
      ...
      case 0:             /* stop all */
        XcpStopDaq();
        if (!XcpTlWaitForTransmitQueueEmpty(1000 /* timeout_ms */))
        { // Wait until transmit queue empty before sending command response
    ...
    case CC_TIME_CORRELATION_PROPERTIES:
    {
      ...
      if ((CRO_TIME_SYNCH_PROPERTIES_SET_PROPERTIES & TIME_SYNCH_SET_PROPERTIES_RESPONSE_FMT) >= 1)
      { // set extended format
...
#ifdef XCP_ENABLE_DAQ_CLOCK_MULTICAST
      if ((CRO_TIME_SYNCH_PROPERTIES_SET_PROPERTIES & TIME_SYNCH_SET_PROPERTIES_CLUSTER_ID) != 0)
      { // set cluster id
      ...
      if ((CRO_TIME_SYNCH_PROPERTIES_GET_PROPERTIES_REQUEST & TIME_SYNCH_GET_PROPERTIES_GET_CLK_INFO) != 0)
...
#define CRO_DWORD(x) (cmdBuf->dw[x])
  ...
  case CC_START_STOP_DAQ_LIST:
    printf(" START_STOP mode=%s, daq=%u\n", (CRO_START_STOP_DAQ_LIST_MODE == 2) ? "select" : (CRO_START_STOP_DAQ_LIST_MODE == 1) ? "start"
                                                                                                                                 : "stop",
           CRO_START_STOP_DAQ_LIST_DAQ);
    break;
  case CC_START_STOP_SYNCH:
    printf(" START_STOP_SYNCH mode=%s\n", (CRO_START_STOP_SYNCH_MODE == 3) ? "prepare" : (CRO_START_STOP_SYNCH_MODE == 2) ? "stop_selected"
