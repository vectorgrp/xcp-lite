...
uint32_t ApplXcpGetId(uint8_t id, uint8_t* buf, uint32_t bufLen) {
    ...
    case IDT_ASAM_PATH:
        ...
        DBG_PRINTF3("GET_ID ASAMP_PATH %s\n", buf);
        break;


#ifdef XCP_ENABLE_IDT_A2L_UPLOAD
    case IDT_ASAM_EPK:
        if (gXcpEpk==NULL) return 0; 
        len = (uint32_t)strlen(gXcpEpk);
        if (buf) {
            if (len > bufLen-1) return 0; // Insufficient buffer space
            strncpy((char*)buf, gXcpEpk, len);
            DBG_PRINTF3("GET_ID EPK %s\n", gXcpEpk);
        }
        else {
            DBG_PRINTF3("GET_ID EPK as upload (len=%u,value=%s)\n", len, gXcpEpk);
        }
        break;
    ...
    case IDT_ASAM_UPLOAD:
        ...
        len = openA2lFile();
        DBG_PRINTF3("GET_ID A2L as upload (len=%u)\n", len);
        break;
#endif
