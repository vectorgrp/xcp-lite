...
extern "C" {
    pub fn XcpTlInit(
        ...
        cb: ::std::option::Option<
            unsafe extern "C" fn(msgLen: u16, msgBuf: *const u8) -> ::std::os::raw::c_int,
        >,
    ) -> ::std::os::raw::c_int;
}
extern "C" {
...
extern "C" {
    pub fn register_callback_set_cal_page(
        cb: ::std::option::Option<unsafe extern "C" fn(segment: u8, page: u8, mode: u8) -> u8>,
    );
}
extern "C" {
    pub fn register_callback_get_cal_page(
        cb: ::std::option::Option<unsafe extern "C" fn(segment: u8, mode: u8) -> u8>,
    );
}
extern "C" {
...
extern "C" {
    pub fn register_callback_init_cal(
        cb: ::std::option::Option<unsafe extern "C" fn(src_page: u8, dst_page: u8) -> u8>,
    );
}
extern "C" {
...
pub fn xcp_short_download(addr: u32, ext: u8, len: u8, data_bytes: &[u8]) -> u8 {
    ...
    unsafe {
        XcpTlCommand(
            4 + 8 + (len as u16),
            &xcp_short_download as *const XcpCmdShortDownload as *const u8,
        )
    }
}
...
extern "C" fn cb_transmit(_msg_len: u16, _msg_buf: *const u8) -> libc::c_int {
    //println!("cb_transmit callback rx {}", msg_len);
    1 // ok
}
...
fn modify_test_u8(calseg: &CalSeg<CalPage>, value: u8) {
    let (ext, addr) = Xcp::get_calseg_ext_addr(
        calseg.get_name(),
        ((&CAL_PAR.test_u8 as *const u8 as usize)
            - (&CAL_PAR as *const CalPage as *const u8 as usize)) as u16,
    );
