...
struct mdfChannelGroup {
    ...
    DWORD recordLen; /* including recordIdLen */
    DWORD actualRecordLen; /* including recordIdLen */
...
static fpos_t mdfDataBlockPos = 0;
static QWORD mdfDataBlockLen = 0;
static QWORD mdfCycleCount = 0;

// Header
static struct mdfHeaderBlock* mdfCreateHeaderBlock( int unfin, LINK64 dataLink, uint32_t recordIdSize) {

    printf("mdfCreateHeaderBlock data=%llu\n", dataLink);
    ...
    h->hdLinks.hd_ch_tree = 0;           /* pointer to (first) channel hierarchy block (CHBLOCK) (can be NIL) */
    h->hdLinks.hd_at_first = 0;          /* pointer to (first) attachment              (ATBLOCK) (can be NIL) */
    h->hdLinks.hd_ev_first = 0;          /* pointer to (first) event block             (EVBLOCK) (can be NIL) */
    h->hdLinks.hd_md_comment = 0;        /* pointer to measurement file comment        (MDBLOCK or TXBLOCK) (can be NIL) */
    ...
    strncpy(h->mdData, 
        ...
        "<common_properties> <e name = \"author\">visza</e> <e name = \"project\">XCPsim2</e> </common_properties> </FHcomment>\r\n", MD_COMMENT_LEN);
    ...
    h->dgLinks.dg_dg_next = 0;                 /* pointer to (next)  data group block    (DGBLOCK) (can be NIL) */
    ...
    h->dgLinks.dg_data = dataLink;                    /* pointer to         data list block (DLBLOCK/HZBLOCK/LDBLOCK) or a data block (DTBLOCK, DVBLOCK or respective DZBLOCK) (can be NIL) */
    h->dgLinks.dg_md_comment = 0;              /* pointer to         TXBLOCK/MDBLOCK with additional comments (can be NIL) */
...
// Channelgroup
static struct mdfChannelGroup* mdfCreateChannelGroupBlock(QWORD recordCount, WORD recordId, DWORD recordLen, LINK64 channelLink) {

    printf("mdfCreateChannelGroupBlock recordCount=%llu id=%u len=%u firstChannel=%llu\n", recordCount, recordId, recordLen, channelLink);
    ...
    h->b.cgLinks.cg_cg_next = 0;;                 /* pointer to next channel group block (CGBLOCK)       (can be NIL)  */
    h->b.cgLinks.cg_cn_first = channelLink;       /* pointer to first channel block (CNBLOCK)            (            must be NIL if MDF4_CG_FLAG_VLSD flag is set) */
    h->b.cgLinks.cg_tx_acq_name = 0;;             /* pointer to TXBLOCK with acquisition name            (can be NIL, must be NIL if MDF4_CG_FLAG_VLSD flag is set) */
    h->b.cgLinks.cg_si_acq_source = 0;;           /* pointer to SIBLOCK with acquisition source info     (can be NIL, must be NIL if MDF4_CG_FLAG_VLSD flag is set) */
    h->b.cgLinks.cg_sr_first = 0;;                /* pointer to first sample reduction block (SRBLOCK)   (can be NIL, must be NIL if MDF4_CG_FLAG_VLSD flag is set) */
    h->b.cgLinks.cg_md_comment = 0;;              /* pointer to TXBLOCK/MDBLOCK with additional comments (can be NIL, must be NIL if MDF4_CG_FLAG_VLSD flag is set) XML schema for contents of MDBLOCK see cg_comment.xsd */
...
static struct mdfChannel *mdfCreateChannelBlock(
   ...
   printf("mdfCreateChannelBlock type=%u next=%llu\n", type, next);
    ...
    c->b.c.cnLinks.cn_cn_next = next;               /* pointer to next CNBLOCK of group (can be NIL) */
    if (dim > 1) {
        c->b.c.cnLinks.cn_composition = (char*)&c->b.a.caHeader - (char*)c;              /* pointer to CABLOCK/CNBLOCK to describe components of this signal (can be NIL) */
    }
    else {
        c->b.c.cnLinks.cn_composition = 0;
    }
    c->b.c.cnLinks.cn_tx_name = (char*)&c->b.c.txHeader - (char*)c; /* relative pointer to TXBLOCK for name of this signal (can be NIL, must not be NIL for data channels) */
    c->b.c.cnLinks.cn_si_source = NIL;              /* pointer to SIBLOCK for name of this signal (can be NIL, must not be NIL for data channels) */
    c->b.c.cnLinks.cn_cc_conversion = (char*)&c->b.c.ccHeader - (char*)c; // relative pointer to conversion rule (CCBLOCK) of this signal (can be NIL) */
    c->b.c.cnLinks.cn_data = 0;                     /* pointer to DLBLOCK/HLBLOCK/SDBLOCK/DZBLOCK/ATBLOCK/CGBLOCK defining the signal data for this signal (can be NIL) */
    c->b.c.cnLinks.cn_md_unit = 0;                  /* pointer to MD/TXBLOCK with string for physical unit after conversion (can be NIL). If NIL, the unit from the conversion rule applies. If empty string, no unit should be displayed. */
    c->b.c.cnLinks.cn_md_comment = 0;               /* pointer to TXBLOCK/MDBLOCK with comment/description of this signal and other information (can be NIL) */
    ...
    c->b.c.ccLinks.cc_md_unit = (char*)&c->b.c.txHeaderUnit - (char*)c;                /* relative pointer to TX/MDBLOCK with string for physical unit after conversion (can be NIL) */
    ...
    c->b.a.caLinks.ca_composition = NIL; /* link to CA or CN block for components or link to CA or CN block for list of structure members */
...
static void mdfAdjustBlockLinks(BLOCK_HEADER* b, BLOCK_HEADER* root, fpos_t pos, uint32_t len) {
    ...
    for (uint32_t i = 0; i < b->link_count; i++) {
        if (l->link[i]!=0) {
            if (l->link[i] < len) {
                mdfAdjustBlockLinks((BLOCK_HEADER*)((char*)root + l->link[i]), root, pos, len);
                l->link[i] += pos;
                 printf("  %u: %llu -> %llu\n", i, l->link[i] - pos, l->link[i]);
            }
            else {
                printf("  %u: %llu\n", i, l->link[i]);
            }
...
static int mdfWriteBlock(FILE * f, BLOCK_HEADER * b, uint32_t len, int update) {
    ...
    fgetpos(f, &pos);
     ...
     printf("mdfWriteBlock %c%c at %llu len=%u\n", 0xFF & (b->id >> 16), 0xFF & (b->id >> 24), pos, len);
    if (update) mdfAdjustBlockLinks(b, b, pos, len);
...
int mdfOpen( const char* filename ) {
    ...
    printf("mdfOpen %s\n", filename);
    if (NULL == (gFile = fopen(filename,"wb"))) {
...
int mdfCreateChannelGroup(uint32_t recordId, uint32_t recordLen, uint32_t timeChannelSize, double timeChannelConv ) {
    ...
    DWORD len = (recordLen <= mdfRecordIdLen) ? 0 : recordLen - mdfRecordIdLen; /*recordLen without id */
    struct mdfChannelGroup * g = mdfCreateChannelGroupBlock(0 /* recordCount*/, recordId /*recordId*/, len /*recordLen without id */, NIL/* channelLink*/);
    if (g == NULL) return 0;
    ...
    g->timeChannel = mdfCreateChannelBlock(TRUE, "Time", MDF4_CN_VAL_UNSIGN_INTEL, 1 /* dim*/, 0 /*byteoffset*/, timeChannelSize, NIL /*next*/, timeChannelConv, 0.0, "s");
    if (g->timeChannel == NULL) return 0;
...
int mdfCreateChannel(const char* name, uint8_t msize, int8_t encoding, uint32_t dim, uint32_t byteOffset, double factor, double offset, const char* unit) {
    ...
    uint32_t mtype;
    switch (encoding) {
        case 1: mtype = MDF4_CN_VAL_UNSIGN_INTEL;  break;
        case -1: mtype = MDF4_CN_VAL_SIGNED_INTEL;  break;
        case 0: mtype = MDF4_CN_VAL_REAL_INTEL;  break;
    default: return 0;
    }
    ...
    struct mdfChannel* c = mdfCreateChannelBlock(FALSE, name, mtype, dim, byteOffset, msize*8, NIL /*next*/, factor, offset, unit);
    if (c == NULL) return 0;
...
int mdfWriteHeader() {
    ...
    QWORD headerSize = sizeof(struct mdfHeaderBlock);
    for (struct mdfChannelGroup* g = mdfChannelGroupFirst; g != NULL; g = g->next) {
        ...
        for (struct mdfChannel* c = g->dataChannelFirst; c != NULL; c = c->next) {
            if (c->b.c.cnLinks.cn_composition != NIL) {
    ...
    if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)mdfHeader, sizeof(struct mdfHeaderBlock), FALSE)) return 0;
    ...
    for (struct mdfChannelGroup* g = mdfChannelGroupFirst; g != NULL; g = g->next) {
            ...
            g->b.cgLinks.cg_cg_next = (g->next==NULL) ? NIL : pos + g->groupHeaderSize; // Next group
            if (g->b.cgData.cg_record_bytes.cg_data_bytes == 0) g->b.cgData.cg_record_bytes.cg_data_bytes = g->actualRecordLen - mdfRecordIdLen;  /* Length of record in Bytes without id */
            g->pos = pos;
            if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)&g->b, sizeof(struct mdfChannelGroupBlock), FALSE)) return 0;
            ...
            if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)&tc->b.c, sizeof(struct mdfChannelBlock), TRUE)) return 0;
            ...
            for (struct mdfChannel* c = g->dataChannelFirst; c != NULL; c = c->next) {
                if (c->b.c.cnLinks.cn_composition != NIL) {
                    ...
                    c->b.c.cnLinks.cn_cn_next = (c->next==NULL) ? NIL : pos+sizeof(struct mdfArrayBlock);
                    if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)&c->b.a, sizeof(struct mdfArrayBlock), TRUE)) return 0;
                    pos += sizeof(struct mdfArrayBlock);
                }
                else {
                    
                    c->b.c.cnLinks.cn_cn_next = (c->next == NULL) ? NIL : pos+sizeof(struct mdfChannelBlock);
                    if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)&c->b.c, sizeof(struct mdfChannelBlock), TRUE)) return 0;
                    pos += sizeof(struct mdfChannelBlock);
                }
            }
    ...
    fgetpos(gFile, &mdfDataBlockPos);
    if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)mdfDataBlock, sizeof(struct mdfDataBlock), FALSE)) return 0;
...
int mdfWriteRecord(const uint8_t* record, uint32_t recordLen) {
    ...
    mdfCycleCount++;
    size_t s = fwrite(record, 1, recordLen, gFile);
    return s == recordLen;
}
...
int mdfClose() {
    ...
    if (mdfHeader != NULL && mdfDataBlock != NULL && mdfChannelGroupLast != NULL) {
        ...
        for (struct mdfChannelGroup* g = mdfChannelGroupFirst; g != NULL; g = g->next) {
            ...
            fsetpos(gFile, (const fpos_t*)&g->pos);
            if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)g, sizeof(struct mdfChannelGroupBlock), FALSE)) return 0;
        }

        // Update data block length
        mdfDataBlock->dtHeader.length = MDF4_DT_MIN_LENGTH + mdfDataBlockLen;
        fsetpos(gFile, &mdfDataBlockPos);
        if (!mdfWriteBlock(gFile, (BLOCK_HEADER*)mdfDataBlock, sizeof(struct mdfDataBlock),FALSE)) return 0;
